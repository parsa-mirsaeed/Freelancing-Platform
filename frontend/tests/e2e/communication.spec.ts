...
test("persisted message send keeps a stable client message id and renders server sequence", async ({ page }) => {
  ...
  await expect(workspace.getByText("I added the latest contract context.")).toBeVisible();
  await expect(workspace.getByText("Cursor sync", { exact: true })).toBeVisible();

  await workspace.getByLabel("Message").fill("The delivery evidence is ready for review.");
...
test("notification read state and channel preference follow backend confirmations", async ({ page }) => {
  ...
  await signIn(page);
  await expect(page.getByText("New contract message")).toBeVisible();
